...
class BatchProcessing:
    ...
    def initialize_logger(self, logger_file_name):
        ...
        handler = TimedRotatingFileHandler(logger_file_name, when="s", interval=10)
        ...
        try:
            ...
            if self.file_path.endswith('.csv'):
                ...
                if count == 0:
                    ...
                    #     csv_data['date'].dt.date.astype(str) + ' ' + csv_data['date'].dt.time.astype(str))

                    # For flipkart data
                    # csv_data.fillna('', inplace=True)
                    csv_data.replace(np.nan, None, inplace=True)
                    csv_data['date_'] = pd.to_datetime(csv_data['date_'])
                    csv_data['date_'] = pd.to_datetime(csv_data['date_'].dt.date.astype(str) + ' ' + csv_data['date_'].dt.time.astype(str))
            ...
            # Check if file is in json or parquet format
                ...
                if count == 0:
                    ...
                    # For amazon data
                    parquet_or_json_data.fillna('', inplace=True)
                    # parquet_or_json_data.replace(np.nan, None, inplace=True)
                    parquet_or_json_data['date'] = pd.to_datetime(parquet_or_json_data['date'])
                    parquet_or_json_data['date'] = pd.to_datetime(
                        parquet_or_json_data['date'].dt.date.astype(str) + ' ' + parquet_or_json_data['date'].dt.time.astype(str))
            ...
            if user_input == 1:
                ...
                if not self.file_path.endswith('.csv')  and not self.file_path.endswith('.parquet') and \
                    ...
                    for i in range(self.number_of_batches):
                        ...
                        # print('Next batch will process after 1 hour')
                        # sleep(3600)

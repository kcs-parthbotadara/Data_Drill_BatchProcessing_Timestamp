...
class BatchProcessing:
    # Constructor
    def __init__(self, user, password, host, database, steps_to_move, number_of_batches, file_path, column_names, batch_scheduling_time,
                 create_table, insert_table, select_table, show_table, truncate_table, drop_table, create_backup_table, insert_backup_table,
                 truncate_backup_table, logger_file_name, email_from, email_password, email_to):
        ...
        self.create_backup_table = create_backup_table
        self.insert_backup_table = insert_backup_table
        self.truncate_backup_table = truncate_backup_table
        ...
        try:
            ...
            if self.file_path.endswith('.csv'):
                ...
                if count == 0:
                    ...
                    # For amazon data
                    # csv_data['date_'] = pd.to_datetime(csv_data['date_'])
                    # csv_data['date_'] = pd.to_datetime(
                    #     csv_data['date_'].dt.date.astype(str) + ' ' + csv_data['date_'].dt.time.astype(str))
                    ...
                    error_message = '-'
                    return status, inserted_rows, error_message
                else:
            ...
            # Check if file is in json or parquet format
                ...
                if self.file_path.endswith('.parquet'):
                    self.logger.info("File is in parquet format")
                    parquet_or_json_data = pd.read_parquet(self.file_path)
                elif self.file_path.endswith('.json'):
                    self.logger.info("File is in json format")
                ...
                if count == 0:
                    ...
                    # For amazon data
                    # parquet_or_json_data['date_'] = pd.to_datetime(parquet_or_json_data['date_'])
                    # parquet_or_json_data['date_'] = pd.to_datetime(
                    #     parquet_or_json_data['date_'].dt.date.astype(str) + ' ' + parquet_or_json_data['date_'].dt.time.astype(str))
...
'''))
            ...
            if user_input == 1:
                ...
                if not self.file_path.endswith('.csv') and not self.file_path.endswith('.parquet') and \
                    ...
                    # Create backup table
                    cursor.execute(self.create_backup_table)
                    self.logger.info("Table created successfully")
                    # Insert table
                    for i in range(self.number_of_batches):
                        ...
                        status, inserted_rows, error_message = self.insert_rows_to_mysql(cursor, i)
                        # Send email
                        end_time = datetime.today().time().replace(second=0, microsecond=0)
                        table_name = self.create_table.split(' ')[2]
                        date = datetime.today().date()
                        if status == 'Failure':
                            message = sent_mail(email_from=self.email_from, email_password=self.email_password,
                                                email_to=self.email_to, batch_size=self.steps_to_move,
                                                start_time=start_time, end_time=end_time, table_name=table_name,
                                                batch_no=i + 1, file_name=self.file_path, date=date,
                                                inserted_rows=inserted_rows, status=status, error_message=error_message)
                            print(f"Batches {i + 1} Failed")
                            ...
                            # Closing connection
                            connection.commit()
                            # Next batch will run after 1 hour
                            # sleep(3600)
...
def get_configuration(config_path):
    ...
    create_backup_table = get_section.get('SQL_QUERY', 'create_backup_table')
    insert_backup_table = get_section.get('SQL_QUERY', 'insert_backup_table')
    truncate_backup_table = get_section.get('SQL_QUERY', 'truncate_backup_table')
    ...
    return username, password, host, database, steps_to_move, number_of_batches, file_path, column_names, batch_scheduling_time, \
           create_table, insert_table, select_table, show_table, truncate_table, drop_table, create_backup_table, insert_backup_table, \
           truncate_backup_table, logger_file_name, email_from, email_to, email_password


if __name__ == '__main__':
    # Get configuration
    config_path = f"{os.getcwd()}\configuration.conf"
    username, password, host, database, steps_to_move, number_of_batches, file_path, column_names, batch_scheduling_time, create_table, \
        insert_table, select_table, show_table, truncate_table, drop_table, create_backup_table, insert_backup_table, truncate_backup_table, \
        logger_file_name, email_from, email_to, email_password = get_configuration(config_path)
    ...
    batch_obj = BatchProcessing(username, password, host, database, steps_to_move, number_of_batches, file_path, column_names, batch_scheduling_time,
                                create_table, insert_table, select_table, show_table, truncate_table, drop_table, create_backup_table,
                                insert_backup_table, truncate_backup_table, logger_file_name, email_from, email_password, email_to)
